...
### Step 0: Vendor Engine Sources (Required)

The blend-space editing commands compile four triangulation helpers from
Unreal's Persona editor module directly into the plugin (they are
module-private in the engine, so they can't be linked). Epic's source may not
be redistributed, so those files are not in this repository — generate them
from your own UE4.27 install before building:

```bash
python scripts/vendor_epic_sources.py "C:/Program Files/Epic Games/UE_4.27"
```

Pass the directory that contains `Engine/`. This copies
`AnimationBlendSpace{,1D}Helpers.{h,cpp}` from
`Engine/Source/Editor/Persona/Private` into the plugin source and applies the
namespace wrapping the plugin needs. The plugin will not compile without this
step.
...
The four `AnimationBlendSpace*Helpers` files generated by
`scripts/vendor_epic_sources.py` are copyright Epic Games, covered by the
Unreal Engine EULA, and are not part of this project's MIT grant. They are
deliberately excluded from this repository; do not commit or redistribute
them.
